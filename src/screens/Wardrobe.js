...
function Wardrobe() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/wardrobe');
        setWardrobeProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>WARDROBE</h2>
        <div className="products" id="products">
