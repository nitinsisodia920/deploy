...
function Woodentemple() {
  const navigate = useNavigate();
  const [WoodentempleProducts, setWoodentempleProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/woodentemple');
        setWoodentempleProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>WOODEN TEMPLE</h2>
        <div className="products" id="products">
          {WoodentempleProducts.map((product) => (
            <div className="product" id="product" key={product.id}>
