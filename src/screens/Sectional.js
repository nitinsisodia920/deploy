...
function Sectional() {
  const navigate = useNavigate();
  const [SectionalProducts, setSectionalProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/sectional');
        setSectionalProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>SECTIONAL SOFA</h2>
        <div className="products" id="products">
          {SectionalProducts.map((product) => (
            <div className="product" id="product" key={product.id}>
