...
function Shoerack() {
  const navigate = useNavigate();
  const [ShoerackProducts, setShoerackProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/shoerack');
        setShoerackProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>SHOE RACK</h2>
        <div className="products" id="products">
          {ShoerackProducts.map((product) => (
            <div className="product" id="product" key={product.id}>
