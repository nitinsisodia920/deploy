...
function Centertable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/centertable');
        setCentertableProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>CENTER TABLES</h2>
        <div className="products" id="products">
          {CentertableProducts.map((product) => (
            <div className="product" id="product" key={product.id}>
