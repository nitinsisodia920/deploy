...
function Luxurious() {
  const navigate = useNavigate();
  const [LuxuriousProducts, setLuxuriousProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('https://deploy-1-w166.onrender.com/api/products/luxurious');
        setLuxuriousProducts(result.data);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="cooler-container">
      <main>
        <h2 className='living-heading'>LUXURIOUS COLLECTION</h2>
        <div className="products" id="products">
          {LuxuriousProducts.map((product) => (
            <div className="product" id="product" key={product.id}>
